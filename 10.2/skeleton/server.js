...
app.get("/all", function(req, res) {
  ...
  db.myanimals.find({}, function(err, data) {
...
app.get("/name", function(req, res) {
  // Query: In our database, go to the animals collection, then sort and show by name
  db.myanimals.find().sort({name:1}, function(err, data) {
    // Log any errors if the server encounters one
    if (err) {
      return console.log(err);
    }
...
app.get("/weight", function(req, res) {
  // Query: In our database, go to the animals collection, then "find" everything
  db.myanimals.find().sort({weight:-1}, function(err, data) {
    // Log any errors if the server encounters one
    if (err) {
      return console.log(err);
    }
